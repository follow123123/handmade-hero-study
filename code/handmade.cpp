...
{
	uint32 LowIndex = AddLowEntity(GameState, EntityType_Hero, &GameState->CameraP); 
	...
	{
		GameState->CameraFollowingEntityIndex = LowIndex;
	}
...
AddWall(game_state *GameState, uint32 AbsTileX, uint32 AbsTileY, uint32 AbsTileZ)
{
	...
	uint32 LowIndex = AddLowEntity(GameState, EntityType_Wall, &P);	
	low_entity *EntityLow = GetLowEntity(GameState, LowIndex);
...
MovePlayer(game_state *GameState, entity Entity, real32 dt, vec2 ddP)
{
	...
	for (uint32 Iteration = 0; Iteration < 4; ++Iteration)
	{
		...
		for (uint32 TestHighEntityIndex = 1; TestHighEntityIndex < GameState->HighEntityCount; ++TestHighEntityIndex)
		{
			...
			{			
				real32 DiameterW = World->TileSideInMeters + Entity.Low->Width;
				real32 DiameterH = World->TileSideInMeters + Entity.Low->Height;
...
// 			if (DoorUp || DoorDown)
		 ...
		 ++HighEntityIndex)
	{
		...
		vec2 PlayerLeftTop = {PlayerGroundPointX - 0.5f*LowEntity->Width*MetersToPixels,
