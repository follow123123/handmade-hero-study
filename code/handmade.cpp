...
inline void
PushPiece(entity_visible_piece_group *PieceGroup, loaded_bitmap *Bitmap,
		  vec2 Offset, real32 OffsetZ, vec2 Align, real32 Alpha=1.0f)
{
	Assert(PieceGroup->PieceCount < ArrayCount(PieceGroup->Pieces));
	entity_visible_piece *Piece = PieceGroup->Pieces + PieceGroup->PieceCount++;
...
MakeEntityHighFrequency(game_state *GameState, low_entity *EntityLow, uint32 LowIndex, vec2 CameraSpaceP)
{
	...
	if (EntityLow->HighEntityIndex == 0)
	{
		if (GameState->HighEntityCount < ArrayCount(GameState->_HighEntities))
		{
			...
			EntityHigh = GameState->_HighEntities + HighIndex;
...
MakeEntityHighFrequency(game_state *GameState, uint32 LowIndex)
{
	...
	{
		EntityHigh = GameState->_HighEntities + EntityLow->HighEntityIndex;
	}
	else
	{
		vec2 CameraSpaceP = GetCameraSpaceP(GameState, EntityLow);
...
inline entity
EntityFromHighIndex(game_state *GameState, uint32 HighIndex)
{
	entity Entity;
	Entity.High = GameState->_HighEntities + HighIndex;
	Entity.LowIndex = Entity.High->LowEntityIndex;
	Entity.Low = GameState->LowEntities + Entity.LowIndex;
...
inline void
OffsetAndCheckFrequencyByArea(game_state *GameState, vec2 Offset, rectangle2 CameraBounds)
{
	for (uint32 HighIndex = 1; HighIndex < GameState->HighEntityCount; )
	{
		high_entity *High = GameState->_HighEntities + HighIndex;
		...
		{
			Assert(GameState->LowEntities[High->LowEntityIndex].HighEntityIndex == HighIndex);
...
AddLowEntity(game_state *GameState, entity_type Type, world_position *P)
{
	...
	*EntityLow = {};
	...
	{
		EntityLow->P = *P;
		ChangeEntityLocation(&GameState->WorldArena, GameState->World, LowIndex, 0, P);
		Assert(GetWorldChunk(GameState->World, P->ChunkX, P->ChunkY, P->ChunkZ)); //@
	}
...
{
	add_low_entity_result Result = AddLowEntity(GameState, EntityType_Hero, &GameState->CameraP); 
	...
	{
		GameState->CameraFollowingEntityIndex = Result.LowIndex;
	}
...
AddWall(game_state *GameState, uint32 AbsTileX, uint32 AbsTileY, uint32 AbsTileZ)
{
	...
	add_low_entity_result Result = AddLowEntity(GameState, EntityType_Wall, &P);	

	Result.Low->Height = GameState->World->TileSideInMeters;
...
{
	world_position P = ChunkPositionFromTilePosition(GameState->World, AbsTileX, AbsTileY, AbsTileZ);
	add_low_entity_result Result = AddLowEntity(GameState, EntityType_Monster, &P);	
...
internal void
MoveEntity(game_state *GameState, entity Entity, real32 dt, vec2 ddP)
{
	...
	for (uint32 Iteration = 0; Iteration < 4; ++Iteration)
	{
		...
		vec2 DesiredPosition = Entity.High->P + PlayerDelta;
		
		for (uint32 TestHighEntityIndex = 1; TestHighEntityIndex < GameState->HighEntityCount; ++TestHighEntityIndex)
		{
			entity TestEntity;
			TestEntity.LowIndex = GameState->_HighEntities[TestHighEntityIndex].LowEntityIndex;
			TestEntity.High = GameState->_HighEntities + TestHighEntityIndex;
...
SetCamera(game_state *GameState, world_position NewCameraP)
{
	...
	rectangle2 CameraBounds = RectCenterDim(vec2{0, 0},
											World->TileSideInMeters*vec2{(real32)TileSpanX, (real32)TileSpanY});
			...
			if (Chunk)
			{
				...
				for (world_entity_block *Block = FirstBlock; Block; Block = Block->Next)
				{
					for (uint32 EntityIndexIndex = 0; EntityIndexIndex < Block->EntityCount; ++EntityIndexIndex)
					{
						...
						{
							vec2 CameraSpaceP = GetCameraSpaceP(GameState, Low);
...
// 			if (DoorUp || DoorDown)
		 ...
		 ++ControllerIndex)
    {
		...
		if (LowIndex == 0)
		{
			if (Controller->Start.EndedDown)
			{
				...
				GameState->PlayerIndexForController[ControllerIndex] = Entity.LowIndex;
			}
		}
		else
		{
		 ...
		 ++HighEntityIndex)
	{
		...
		switch (LowEntity->Type)
		{
		    ...
		    {
		    	PushPiece(&PieceGroup, &GameState->Shadow, Vec2(0, 0), 0, HeroBitmaps->Align, ShadowAlpha);
		    	PushPiece(&PieceGroup, &HeroBitmaps->Torso, Vec2(0, 0), 0, HeroBitmaps->Align);
		    	PushPiece(&PieceGroup, &HeroBitmaps->Cape, Vec2(0, 0), 0, HeroBitmaps->Align);
		    	PushPiece(&PieceGroup, &HeroBitmaps->Head, Vec2(0, 0), 0, HeroBitmaps->Align);
		    } break;
		    ...
		    case EntityType_Familiar:
		    {
		    	...
		    	PushPiece(&PieceGroup, &GameState->Shadow, Vec2(0, 0), 0, HeroBitmaps->Align, ShadowAlpha);
		    	PushPiece(&PieceGroup, &HeroBitmaps->Head, Vec2(0, 0), 10.f*Sin(2.0f*Entity.High->tBob), HeroBitmaps->Align);
		    } break;
			
		    case EntityType_Wall:
		    {
		    	PushPiece(&PieceGroup, &GameState->Tree, Vec2(0, 0), 0, Vec2(40, 80));
		    } break;
