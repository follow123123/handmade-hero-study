...
internal void
MoveEntity(game_state *GameState, entity Entity, real32 dt, move_spec *MoveSpec, vec2 ddP)
{
...
UpdateSword(game_state *GameState, entity Entity, real32 dt)
{
	...
	vec2 OldP = Entity.High->P;
	MoveEntity(GameState, Entity, dt, &MoveSpec, Vec2(0, 0));
		...
		else
		{
			...
			if ((dSword.X != 0.0f) || (dSword.Y != 0.0f))
			{
				...
				if (LowSword && !IsValid(LowSword->P))
				{
					...
					entity HighSword = ForceEntityIntoHigh(GameState, ControllingEntity.Low->SwordLowIndex);
					HighSword.Low->DistanceRemaining = 5.0f;
					HighSword.High->dP = 5.0f*dSword;
				}
			}
		}
	}	
